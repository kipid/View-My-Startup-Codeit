...
function SelectComparisonModal({ comparisons, onClose }) {
	...
	const [selectedCompanies, setSelectedCompanies] = useState(alreadySelectedIds);
	...
	const searchedCompanies = search
		? companies.filter(company => company.name.includes(search) || company.category.includes(search))
		: companies;
	...
	const handleClick = () => {
		const myComparisons = companies.filter(company => selectedCompanies.includes(company.id));
	...
	}, [search, searchTotal, searchedCompanies]);
